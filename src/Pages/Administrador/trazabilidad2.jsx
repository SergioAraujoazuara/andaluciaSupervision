...
function FormularioInspeccion({setModalFormulario, currentSubactividadId,ppiSelected}) {
    const {id} = useParams()
    ...
    const enviarDatosAFirebase = async () => {
        console.log(currentSubactividadId, 'ppi');
        if (!ppiSelected || !ppiSelected.docId || !currentSubactividadId) {
            console.error("Faltan datos necesarios para actualizar la subactividad.");
            return;
        }
        ...
        const datosDelFormulario = {
            ...
            obra,
            tramo,
            ppiSelected: ppiSelected.nombre, // Asegúrate de que esto tenga un valor válido.
            plano,
            observaciones,
            sector: sector || '',
            subSector: subSector || '',
        ...
        try {
            const docRefRegistro = await addDoc(collection(db, "registros"), datosDelFormulario);
            console.log("Documento de registro añadido con ID: ", docRefRegistro.id);
    
            // Identificar la actividad y subactividad específicas usando el ID actual
            const [actividadIndex, subactividadIndex] = currentSubactividadId.split('-').map(x => parseInt(x, 10));
    
            // Preparar la actualización marcando la subactividad específica como enviada
            const updatedActividades = ppiSelected.actividades.map((actividad, index) => {
                if (index === actividadIndex) {
                    const updatedSubactividades = actividad.subactividades.map((sub, sIndex) => {
                        if (sIndex === subactividadIndex) {
                            const updatedSubactividad = { ...sub, formularioEnviado: true }; // Marcar esta subactividad como enviada
                            console.log('Subactividad actualizada para console log:', updatedSubactividad); // Aquí agregamos el console log
                            return updatedSubactividad;
                        }
                        return sub;
                    });
                    return { ...actividad, subactividades: updatedSubactividades };
                }
                return actividad;
            });
    
            // Actualizar el documento de Firestore con las actividades actualizadas
            await updateDoc(doc(db, "lotes", idLote, "inspecciones", ppiSelected.docId), {
                actividades: updatedActividades
            });
    
            console.log("Subactividad actualizada con éxito para marcar el formulario como enviado.");
            setModalFormulario(false); // Cerrar modal o notificar al usuario
        } catch (error) {
            console.error("Error al enviar datos del formulario y actualizar subactividad:", error);
        }
    };
...
// Colocar texto dentro del recuadro
doc.text(`PPI: ${obra}`, 15, 60);
doc.text(`Plano que aplica: ${tramo}`, 15, 65);
...
doc.text('Resultado de la inspección', 15, 250);
doc.text('Aplica', 15, 260);
doc.text('No Aplica', 40, 260);
doc.text('Firma', 150, 260);
                ...
                <Link to={'#'}>
                    ...
                    <form className="bg-white text-gray-500  mb-4">
                        <div className='grid sm:grid-cols-1 grid-cols-1 gap-4'>
                            <h2 className='text-xl font-medium'>Formulario inspección</h2>
                            {/* Campos de entrada */}
                        ...
                        <div className='flex gap-5'>
                            <button type="button" onClick={enviarDatosAFirebase} className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Guardar</button>
                            <button  type="button" className="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline flex gap-2 items-center" onClick={generatePDF}><FaFilePdf />Informe </button>
                        </div>
                    </form>
